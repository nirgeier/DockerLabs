...
update_yaml_field_if_empty() {
    ...
    # Check if field exists but is empty (including just whitespace or empty quotes after colon)
    elif grep -qE "^${field_name}:[[:space:]]*$" "$config_file"; then
        ...
        local current_value
        current_value=$(grep "^${field_name}:" "$config_file" | head -1)
        if [[ "$VERBOSE" == true ]]; then
            print_warning "$field_name already has a value: $current_value"
        fi
    fi
}
